"""Measure Probability Matching: each pool driven by an adaptive strategy, per variant.

Sweeps each pool variant (full and reduced) of every representation under adaptive
operator selection -- one
:class:`~aos_ga.variation.adaptive_operator.AdaptiveOperatorStep` letting a
:class:`~aos_ga.aos.probability_matching.ProbabilityMatching` strategy choose the
operator and crediting it with the instant reward -- across the grid ``instances x
POPULATION_SIZES x GENERATION_BUDGETS`` over the study's repetition seeds, and takes the
median (with the mean, sample std, min and max) of each variant's final quality.

Unlike the sweeps under ``baselines``, this measures a *treatment*, not a reference
point: the fixed-best operator is the ceiling, Random selection the floor, and the
adaptive strategies are what the study is actually about. The schema is nevertheless
identical to the baselines' down to the column order, so the three levels join on
``(problem, instance_id, population_size, generations, pool_variant)`` with no reshaping
...
``instance_id`` is the problem's own name. A sweep cell's unit is a pool variant, not an
operator: the whole pool is one configuration, since choosing within it is the mechanism
under test. The run emits two artifacts, the raw per-run rows and the per-pool-variant
statistics, both keyed by the pool variant. The strategy's own dynamics (probabilities
and operator choices over time) are deliberately not logged here -- this sweep measures
solution quality; the dynamics log is a separate concern.
"""
...
def run_pm(
    ...
    *,
    ...
    """One GA run on ``problem`` from ``seed`` with ``pool`` selected adaptively.

    Builds a **fresh** strategy and a fresh variation step on every call. That is the
    contract of this function, not an implementation detail: the quality estimates are
    run state, so a strategy shared across seeds would let each repetition start from
    what the previous ones learned -- nothing would fail, the results would simply stop
    measuring what they claim to. The strategy's arms are read off the pool, so the two
    cannot drift apart. Runs the engine with its defaults (tournament ``k=3``, one elite)
    from a fresh ``run_generator(seed)``; the caller reads ``best_quality``.
    """
    strategy = ProbabilityMatching([operator.operator_id for operator in pool])
    adaptive_step = AdaptiveOperatorStep(pool, strategy)
...
def write_aggregate_csv(path: Path, results: list[PoolResult]) -> None:
    """Write the per-pool-variant statistics (one line per variant, aggregates of best_quality)."""
    with path.open("w", newline="") as csvfile:
        writer = csv.DictWriter(csvfile, fieldnames=_AGGREGATE_CSV_COLUMNS)
        writer.writeheader()
        for result in results:
            writer.writerow(
                {
...
def main() -> None:
    ...
    argument_parser = argparse.ArgumentParser(
        description="Measure Probability Matching over both pool variants and report the results."
    )
    ...
    if args.agg_csv:
        write_aggregate_csv(Path(args.agg_csv), pool_results)
        print(f"Wrote per-variant stats: {args.agg_csv}", file=sys.stderr)
